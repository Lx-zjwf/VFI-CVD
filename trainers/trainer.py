...
class Train_Manager:
    ...
    def train(self, model, vlm, preprocess):
        ...
        for e in tqdm(range(total_epoch)):
            ...
            iter_counter, train_cif_acc, train_vd_acc = (
                train_func(model=model, vlm=vlm, preprocess=preprocess,
                           optimizer=optimizer, writer=writer, iter_counter=iter_counter))

            if (e + 1) % args.val_epoch == 0:

                logger.info("")
                logger.info("epoch %d/%d, iter %d:" % (e + 1, total_epoch, iter_counter))
                logger.info("train_cif_acc: %.3f" % (train_cif_acc))
                logger.info("train_vd_acc: %.3f" % (train_vd_acc))
                ...
                with torch.no_grad():
                    ...
                    writer.add_scalar('val_%d-way-%d-shot_acc' % (test_way, val_shot), val_acc, val_interval)
    ...
    def evaluate(self, model, vlm, preprocess):
        ...
        with torch.no_grad():
            ...
            for shot in args.test_shot:
                ...
                logger.info('%d-way-%d-shot acc of cif: %.3f\t%.3f' % (args.test_way, shot, mean, interval))
