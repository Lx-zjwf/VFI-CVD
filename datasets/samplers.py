...
class random_sampler(Sampler):
    ...
    def __iter__(self):
        ...
        for i in range(trial):
            ...
            for cat in picked_class:
                query_list = class2id[cat][shot:(shot + query_shot)]
                if len(query_list) < query_shot:
                    print('insufficient query!')
                    query_list = np.random.choice(query_list, query_shot).tolist()
                id_list.extend(query_list)
